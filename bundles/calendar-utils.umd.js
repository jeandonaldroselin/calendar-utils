...
/* harmony export */   "getResourceWeekView": () => (/* binding */ getResourceWeekView),
...
function getDayView(dateAdapter, _a) {
    var events = _a.events, keepAllDay = _a.keepAllDay, viewDate = _a.viewDate, hourSegments = _a.hourSegments, dayStart = _a.dayStart, dayEnd = _a.dayEnd, eventWidth = _a.eventWidth, segmentHeight = _a.segmentHeight, hourDuration = _a.hourDuration, minimumEventHeight = _a.minimumEventHeight;
    ...
    var eventsInPeriod = getEventsInPeriod(dateAdapter, {
        events: keepAllDay
            ? events
            : events.filter(function (event) { return !event.allDay; }),
...
function getResourceWeekView(dateAdapter, _a) {
    var _b = _a.events, events = _b === void 0 ? [] : _b, _c = _a.resources, resources = _c === void 0 ? [] : _c, viewDate = _a.viewDate, weekStartsOn = _a.weekStartsOn, _d = _a.excluded, excluded = _d === void 0 ? [] : _d, _e = _a.precision, precision = _e === void 0 ? 'days' : _e, _f = _a.absolutePositionedEvents, absolutePositionedEvents = _f === void 0 ? false : _f, hourSegments = _a.hourSegments, dayStart = _a.dayStart, dayEnd = _a.dayEnd, weekendDays = _a.weekendDays, segmentHeight = _a.segmentHeight, minimumEventHeight = _a.minimumEventHeight, _g = _a.viewStart, viewStart = _g === void 0 ? dateAdapter.startOfWeek(viewDate, { weekStartsOn: weekStartsOn }) : _g, _h = _a.viewEnd, viewEnd = _h === void 0 ? dateAdapter.endOfWeek(viewDate, { weekStartsOn: weekStartsOn }) : _h, _j = _a.keepUnassignedEvents, keepUnassignedEvents = _j === void 0 ? true : _j, _k = _a.unassignedRessourceName, unassignedRessourceName = _k === void 0 ? 'Unassigned' : _k;
    if (!events) {
        events = [];
    }
    if (!resources) {
    ...
    var resourcesMaxRowsNumber = [];
    var setResourcesMaxRowsNumber = function (result) {
        resourcesMaxRowsNumber = result;
    };
    var rowColumns = getResourceWeekViewHourGrid(dateAdapter, {
        events: events,
        resources: resources,
        viewDate: viewDate,
        hourSegments: hourSegments,
...
function getResourceWeekViewHourGrid(dateAdapter, _a, setResourcesMaxRowsNumber) {
    var events = _a.events, resources = _a.resources, viewDate = _a.viewDate, hourSegments = _a.hourSegments, hourDuration = _a.hourDuration, dayStart = _a.dayStart, dayEnd = _a.dayEnd, weekStartsOn = _a.weekStartsOn, excluded = _a.excluded, weekendDays = _a.weekendDays, segmentHeight = _a.segmentHeight, viewStart = _a.viewStart, viewEnd = _a.viewEnd, minimumEventHeight = _a.minimumEventHeight, keepUnassignedEvents = _a.keepUnassignedEvents, unassignedRessourceName = _a.unassignedRessourceName;
    var weekDays = getWeekViewHeader(dateAdapter, {
        viewDate: viewDate,
        weekStartsOn: weekStartsOn,
        excluded: excluded,
    ...
    return weekDays.map(function (day) {
        ...
        var mappedEvents = function (calendarEvents) {
            return calendarEvents.map(function (event, index) {
                return {
                    event: event,
                    height: segmentHeight,
                    left: 0,
        ...
        var accumulatePreviousContainerCountsByIndex = function (currentContainerIndex) {
            var count = 0;
            for (var k = 0; k < currentContainerIndex; k++) {
                count += resourcesMaxRowsNumber[k].count;
            }
            return count;
        };
        var getDayEventsGroupedByResource = function (eventList, resourceList, keepUnassignedEvents, unassignedRessourceNameLocal) {
            var unknwownResource = {
                id: undefined,
                name: unassignedRessourceNameLocal,
            };
            var combinedResourceList = keepUnassignedEvents === true
                ? __spreadArray(__spreadArray([], resourceList, true), [unknwownResource], false) : resourceList;
            return combinedResourceList.map(function (resource, resourceIndex) {
                var _a;
                var regularResourceFilterClosure = function (event) {
                    return event.resources &&
                        event.resources.some(function (oneResource) {
                            return oneResource.id === resource.id;
                        });
                };
                var unknownResourceFilterClosure = function (event) {
                    return !event.resources || event.resources.length === 0;
                };
                var filterClosure = resource.id !== undefined
                    ? regularResourceFilterClosure
                    : unknownResourceFilterClosure;
                var filteredEvents = eventList.filter(filterClosure);
                var resourcesMaxRowsNumberItem = resourcesMaxRowsNumber[resourceIndex];
                var isCountGreaterThanPrevious = !!resourcesMaxRowsNumberItem &&
                    resourcesMaxRowsNumberItem.count >= filteredEvents.length;
                var newCount = isCountGreaterThanPrevious
                    ? resourcesMaxRowsNumberItem.count
                    : filteredEvents.length;
                resourcesMaxRowsNumber[resourceIndex] = __assign(__assign({}, resourcesMaxRowsNumberItem), { count: newCount + (resourceIndex >= 0 && newCount == 0 ? 1 : 0), resource: resource, top: accumulatePreviousContainerCountsByIndex(resourceIndex) *
                        segmentHeight });
                return {
                    resource: resource,
                    resourceCurrentDayEventNumber: (_a = filteredEvents.length) !== null && _a !== void 0 ? _a : 1,
        ...
        return {
            date: day.date,
            eventsGroupedByResource: getDayEventsGroupedByResource(dayView.events.map(function (timeEvent) { return timeEvent.event; }), resources, keepUnassignedEvents, unassignedRessourceName),
        };
    });
}
